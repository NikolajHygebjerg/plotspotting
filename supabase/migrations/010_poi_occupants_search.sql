-- Søg også i flere beboere/virksomheder per sted (metadata.occupants JSON-array)
...
AS $$
  ...
  WHERE e.public_slug = p_slug
    ...
    AND (
      ...
      EXISTS (
        SELECT 1
        FROM jsonb_array_elements(coalesce(p.metadata->'occupants', '[]'::jsonb)) elem
        WHERE coalesce(elem->>'name', '') ILIKE '%' || p_query || '%'
      )
    );
$$;
